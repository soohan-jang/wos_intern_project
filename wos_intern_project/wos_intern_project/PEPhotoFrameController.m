...
@implementation PEPhotoFrameMessageSender
...
@implementation PEPhotoFrameController
...
- (instancetype)initWithCollectionViewSize:(CGSize)size {
    ...
    if (self) {
        ...
        for (int i = 0; i < NumberOfPhotoFrameCells; i++) {
            [self.cellDatas addObject:[[PEPhotoFrame alloc] initWithIndexPath:[NSIndexPath indexPathForItem:i inSection:0]]];
        }
...
- (void)setEnableCells:(BOOL)enabled {
    ...
    for (PEPhotoFrame *data in _cellDatas) {
